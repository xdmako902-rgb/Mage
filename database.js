...
const connectdb = async () => {
    try {
        ...
        console.log("✅ Database Connected Successfully | ඩේටාබේස් එක කනෙක්ට් වුණා");
    } catch (e) {
        console.error("❌ Database Connection Failed | ඩේටාබේස් කනෙක්ෂන් වැරදියි:", e.message);
    }
};
...
const userConfigSchema = new mongoose.Schema({
    ...
    config: {
        ...
        REJECT_MSG: { type: String, default: '*🔕 කෝල් එකක් ගහන්න එපා බන් 😒🌹*\n\n📞 Owner: PODI MAKO\n🔰 Bot: MAKO MD MINI BOT' },
        ...
        AUTO_STATUS_MSG: { type: String, default: '🤗 MAKO MD බොට් ස්ටේටස් බලාගෙන යනවා 😊' },
...
async function saveSessionToMongoDB(number, credentials) {
    try {
        ...
        console.log(`📁 Session saved to MongoDB for ${cleanNumber} | සෙෂන් එක සේව් වුණා`);
        return true;
    } catch (error) {
...
async function deleteSessionFromMongoDB(number) {
    try {
        ...
        console.log(`🗑️ Session deleted from MongoDB for ${cleanNumber} | සෙෂන් එක ඩිලීට් වුණා`);
        return true;
    } catch (error) {
    ...
    try {
        ...
        if (config) {
            ...
            const defaultConfig = {
                ...
                AUTO_TYPING: 'false',
                ANTI_CALL: 'false',
                REJECT_MSG: '*🔕 කෝල් එකක් ගහන්න එපා බන් 😒🌹*\n\n📞 Owner: MAKO MD\n🔰 Bot: MAKO MD MINI BOT',
                READ_MESSAGE: 'false',
                AUTO_VIEW_STATUS: 'true',
                AUTO_LIKE_STATUS: 'true',
                AUTO_STATUS_REPLY: 'false',
                AUTO_STATUS_MSG: '🤗 MAKO MD බොට් ස්ටේටස් බලාගෙන යනවා 😊',
                AUTO_LIKE_EMOJI: ['❤️', '👍', '😮', '😎', '🔥', '🇱🇰']
            };
...
async function updateUserConfigInMongoDB(number, newConfig) {
    try {
        ...
        console.log(`⚙️ Config updated for ${cleanNumber} | කන්ෆිග් අප්ඩේට් වුණා`);
        return true;
    } catch (error) {
...
async function saveOTPToMongoDB(number, otp, config) {
    try {
        ...
        console.log(`🔐 OTP saved for ${cleanNumber} | ඕටීපී එක සේව් වුණා`);
        return true;
    } catch (error) {
...
async function verifyOTPFromMongoDB(number, otp) {
    try {
        ...
        if (!otpRecord) {
            return { valid: false, error: 'Invalid or expired OTP | වැරදි හෝ කල් ඉකුත් වූ OTP' };
        }
        ...
        return { valid: false, error: 'Verification error | වෙරිෆයි වැරදියි' };
    }
}
